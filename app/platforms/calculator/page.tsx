...
export default function CalculatorPage() {
  ...
  const { rows, hasValidRate } = useMemo(() => {
    ...
    // Filter only valid numeric vendors
    const validVendors = safeVendors.filter(
      (v) => typeof v.rate === "number" && v.rate > 0
    );
  ...
  const sortedRows = useMemo(() => {
    ...
    const selected = rows.find((r) => r.vendor === platformSelected);
    const others = rows.filter((r) => r.vendor !== platformSelected);
    return selected ? [selected, ...others] : rows;
  }, [rows, platformSelected]);
  ...
  const selectedRow = sortedRows.find((r) => r.vendor === platformSelected);
    ...
    <main className="p-6 bg-bg text-foreground">
      ...
      <div className="flex flex-wrap gap-4 items-end mb-6">
        <div className="flex flex-col gap-1">
          <label htmlFor="amount-input" className="text-sm font-medium text-muted">Amount</label>
          <input
            id="amount-input"
            type="number"
            value={amountInput}
            onChange={(e) => onAmountChange(e.target.value)}
            className="border border-muted bg-surface text-bg p-2 rounded w-40"
            aria-label="Amount"
          />
        </div>
        <div className="flex flex-col gap-1">
          <label htmlFor="fiat-select" className="text-sm font-medium text-primary">Currency</label>
          <select
            id="fiat-select"
            value={fiat}
            onChange={(e) => setFiat(e.target.value as "usd" | "ngn")}
            className="border border-muted bg-surface text-black p-2 rounded"
          >
            <option value="usd">USD</option>
            <option value="ngn">NGN</option>
          </select>
        ...
        <div className="flex flex-col gap-1">
          <label htmlFor="platform-select" className="text-sm font-medium text-muted">Platform</label>
          <select
            id="platform-select"
            value={platformSelected}
            onChange={(e) => setPlatformSelected(e.target.value)}
          ...
          >
            {safeVendors.map((v) => (
              <option key={v.name} value={v.name} className="bg-bg text-black">
                {v.name} {v.rate ? `(${v.rate.toFixed(2)})` : "(no rate)"}
              </option>
            ))}
          </select>
        </div>
        <div className="flex flex-col gap-1">
          <label htmlFor="coin-select" className="text-sm font-medium text-muted">Crypto</label>
          <select
            id="coin-select"
            value={selectedCoinSymbol}
            onChange={(e) => setSelectedCoinSymbol(e.target.value)}
          ...
          >
            {geckoCoins.slice(0, 50).map((c) => (
              <option key={c.id} value={c.symbol.toUpperCase()} className="bg-bg text-black">
                {c.name} ({c.symbol.toUpperCase()})
              </option>
            ))}
          </select>
      ...
      {/* ✅ Selected Platform Summary */}
      {selectedRow && (
        <div className="card mb-6 text-bg border border-muted/30">
          <strong className="text-accent">Selected Platform:</strong>{" "}
          {selectedRow.vendor}
          <div className="text-sm text-muted mt-1">
            USD equivalent: $
